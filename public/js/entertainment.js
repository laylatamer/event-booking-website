(function() {
    ...
    let activeFilter = {
        date: 'All Dates',
        category: 'All Entertainment',  // Changed from 'All Categories'
        venue: 'All Venues'
    };
    ...
    /**
     * Fetch entertainment events from API
     */
    async function fetchEntertainmentEvents() {
        try {
            const response = await fetch(`${API_BASE}?action=getByCategory&category_name=Entertainment`);
            if (!response.ok) {
    ...
    /**
     * Fetch entertainment subcategories
     */
    async function fetchEntertainmentSubcategories() {
        try {
            ...
            console.error('Error fetching entertainment subcategories:', error);
            return [];
        }
    }
    ...
    function updateFilterButtonStyles() {
        ...
        filters.forEach(filterType => {
            ...
            if (filterType === 'category') defaultText = 'All Entertainment';
    ...
    function renderEventCards() {
        ...
        const filteredEvents = allEvents.filter(event => {
            ...
            // 2. Filter by Category (Entertainment type)
            const categoryMatch = activeFilter.category === 'All Entertainment' || 
        ...
        if (filteredEvents.length === 0) {
            grid.innerHTML = `<div class="loading-indicator">No entertainment events match your current filter selection. Try resetting filters.</div>`;
            return;
        }
            ...
            contentEl.innerHTML = `
                <div class="blurb-text-box">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem; margin: 1rem 0;">
                        ...
                        <div style="background: #1a1a1a; padding: 0.75rem; border-radius: 6px;">
                            <p style="font-size: 0.875rem; color: #9ca3af; margin-bottom: 0.25rem;">Event Type</p>
                            <p style="font-weight: 600;">${event.subcategory || 'Entertainment'}</p>
                        </div>
    ...
    /**
     * Populates entertainment subcategories modal with dynamic data
     */
    async function populateEntertainmentSubcategoriesModal() {
        const container = document.getElementById('category-list-container');
        if (!container) return;
        
        const subcategories = await fetchEntertainmentSubcategories();
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        const grid = document.getElementById('events-grid');
        if (grid) {
            grid.innerHTML = '<div class="loading-indicator">Loading entertainment events...</div>';
        }
        ...
        await populateEntertainmentSubcategoriesModal();
